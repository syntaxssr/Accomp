...
| `/en`, `/th` | 200, Phase 2.3 marker, matching `lang`, one H1, complete hash targets, no form |
| `/en/roadmap`, `/th/roadmap` | 200 with matching `lang`, one H1, six semantic mobile-app stages and one current step |
| `/en/support`, `/th/support` | 200 with matching `lang`, one H1, inactive support status, intentional empty state and no form |
...
| `/sitemap.xml` | 200 with EN/TH homepage, Roadmap, Support, Privacy and Terms URLs |
| Missing EN/TH routes | 404 with matching `lang` and `noindex` |

The sixteen-request matrix also requires the homepage, roadmap, Support, legal
pages, localized 404 pages and health response to include the critical
